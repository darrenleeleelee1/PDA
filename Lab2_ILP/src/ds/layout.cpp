...
int64_t SP::countCost(Layout* layout, double alpha_ratio){
    this->calMatch();
    this->MWCS();
    ...
    if(layout->width < layout->chip_width) cost += (layout->chip_width / layout->width) * 1000000000000;
    if(layout->height < layout->chip_height) cost += (layout->chip_height / layout->height) * 1000000000000;
    return cost;
}
...
void Layout::SA(int k)
{
    std::srand(unsigned(std::time(0)));
    double T = 4000000000, T_frozen = 100, r = 0.9;
    this->sp->randomSP();   
    int64_t old_cost = this->sp->countCost(this, 1), new_cost;
    while(T >= T_frozen){
        int n1, n2, op, action, action_rot = -1;
        op = std::rand() % 100;
        ...
        for(int _k = 0; _k < k; _k++){
            ...
            if(this->opt_cost > new_cost && (this->chip_width <= this->width && this->chip_height <= this->height)) std::cout << "ok" << std::endl;
